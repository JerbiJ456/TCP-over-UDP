...
def main():
    ...
    class Client(Thread):
        ...
        def run(self):
            ...
            timeout = 0.01
            rtt = 0.05
            taille_fenetre_init = 400
            aug_taille_fenetre = 50
            taille_fenetre = taille_fenetre_init
            dernier_ack = 0
            nombre_client = 3000
            SIZE_BUFFER = 1494 #Taille du buffer
            rtt_moy = 0.005
            ...
            print(f'Taille fichier {stats.st_size / (1024 * 1024)} Débit : {(stats.st_size / (1024 * 1024))/(time.time()-start)}')
            print("File send")
